...
// ======================================= >> Code Starts Here << ========================== //
/**
 * Global lookup for the organization type picked on step 2 of the setup wizard
 * (ស្ថាប័នរដ្ឋ, ក្រុមហ៊ុនឯកជន, …). Not organization-scoped — every organization
 * chooses from the same list.
 */
@Entity({ name: 'organization_type', schema: 'organization' })
export class OrganizationTypeEntity {
    ...
    @Column({ type: 'varchar', length: 255, nullable: true }) description_kh: string;
    @Column({ type: 'varchar', length: 255, nullable: true }) description_en: string;

    /** Artwork on the file service, seeded by OrganizationTypeSeeder. */
    ...
    @OneToMany(() => Organization, (organization) => organization.type) organizations: Organization[];
}
